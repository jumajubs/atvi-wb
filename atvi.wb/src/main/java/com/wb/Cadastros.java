...
public class Cadastros extends Cadastro{
	...
	public void cadastrar() {
		...
		while (masc < 17) 
		{
			...
			int prod = 0;
			ProdConsumido prodCons = new ProdConsumido((empresa.getProdutos().get(prod)), masc);
			...
			ProdConsumido prodCons1 = new ProdConsumido((empresa.getProdutos().get(prod+1)), masc);
			empresa.getClientes().get(masc).getProdutosConsumidos().add(prodCons1);
			
			ServConsumido servCons = new ServConsumido((empresa.getServicos().get(prod)), masc);
		...
		while (fem < 30) {
			...
			int prod = 0;
			ProdConsumido prodCons = new ProdConsumido((empresa.getProdutos().get(prod)), fem);
			empresa.getClientes().get(fem).getProdutosConsumidos().add(prodCons);
			String nomeP2 = "Produto" + (fem+2);
			double precoP2 = fem+2;
			Produto produto2 = new Produto(nomeP2,precoP2);
			this.produtos.add(produto2);
			ProdConsumido prodCons1 = new ProdConsumido((empresa.getProdutos().get(prod+1)), fem);
			empresa.getClientes().get(fem).getProdutosConsumidos().add(prodCons1);
			
			ServConsumido servCons = new ServConsumido((empresa.getServicos().get(prod)), fem);
